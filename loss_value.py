...
def calc_portfolio_value_change(price_change, weights, init_weights):
	rate_return = tf.reduce_sum(tf.multiply(price_change, init_weights), axis=1)
	transaction_decay = calc_transaction_decay(price_change, weights, init_weights)
...
def calc_transaction_decay(price_change, weights, init_weights):
	...
	norm = tf.norm(tf.sqrt(weights_prime), axis=1)
	print('Shape of norm = ' ,norm.get_shape())
	tmp = tf.constant(12, shape=[1])
	print('Shape of tmp = ', tmp.get_shape())
	norm = tf.tile(norm, tmp)
	print('Shape of tiled norm = ', norm.get_shape())
	norm = tf.reshape(norm, [200, 12])
	# norm = tf.reshape(tf.tile(norm, tmp), 
	#  	[tf.shape(weights_prime)[1], tf.shape(weights_prime)[0]])
	print('Shape of reshaped tiled norm = ', norm.get_shape())
	weights_prime = tf.divide(weights_prime, norm)
